...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --base)
            ...
            ;;
        --help|-h) 
            echo "Usage: $0 [--json] [--short-name <name>] [--number N] [--base <branch>] <feature_description>"
            echo ""
            echo "Options:"
            echo "  --json              Output in JSON format"
            echo "  --short-name <name> Provide a custom short name (2-4 words) for the branch"
            echo "  --number N          Specify branch number manually (overrides auto-detection)"
            echo "  --base <branch>     Create feature branch from this base (default: main)"
            echo "  --help, -h          Show this help message"
            echo ""
            echo "Examples:"
            echo "  $0 'Add user authentication system' --short-name 'user-auth'"
            echo "  $0 'Implement OAuth2 integration for API' --number 5"
            echo "  $0 'Implement OAuth2 integration for API' --base main"
            exit 0
            ;;
        *) 
            ARGS+=("$arg") 
...
# Resolve deterministic base ref for feature-branch creation.
# Priority:
# 1) --base argument
# 2) SPECIFY_BASE_BRANCH environment variable
# 3) main / origin/main / master / origin/master
resolve_base_ref() {
    local requested="$1"
    local candidate
    ...
    if [ -n "$requested" ]; then
        ...
        if git rev-parse --verify --quiet "origin/${requested}^{commit}" >/dev/null; then
