...
[Brief description of what this pipeline validates end-to-end]
...
```bash
# Full E2E flow
scripts/e2e_<slug>.sh full <config>

# Preflight only (dry-run, no external deps needed beyond the app)
scripts/e2e_<slug>.sh preflight <config>

# Run specific user story section
scripts/e2e_<slug>.sh run <config>

# Print verification commands
scripts/e2e_<slug>.sh verify <config>

# CI-safe non-interactive checks only
scripts/e2e_<slug>.sh ci <config>
```
