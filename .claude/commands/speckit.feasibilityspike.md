...
Prove that the architecture committed to in plan.md is actually buildable before any task breakdown or LLD begins. A failed spike routes back to `/speckit.plan` for architecture revision. `/speckit.solution` hard-blocks until this command clears all Open Feasibility Questions.
