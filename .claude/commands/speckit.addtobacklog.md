...
7. **Determine next-phase handoff command** (deterministic triage output):
   - Select exactly one next command based on triage + estimate outcomes:
     ...
     - **Route to `/speckit.solution`** when architecture fits but the new/changed tasks are non-trivial (any added task estimated ≥3 points, cross-surface integration, or contract/seam changes).
     - **Route to `/speckit.implement`** only when all added tasks are small (≤2 points), architecture is unchanged, and required solution artifacts/HUDs are already present and current.
   - Record the selected command as `NEXT_COMMAND` for the final handoff message.
   - Derive `READINESS_EVENT` from `NEXT_COMMAND`:
     - `/speckit.specify` -> `backlog_registered`
     - `/speckit.plan` -> `research_completed`
     - `/speckit.solution` -> `plan_approved`
...
9. **Emit pipeline events for readiness projection**:
   ...
   - Build projected prerequisite chain in canonical order:
     ...
     - `analysis_completed`
     - `e2e_generated`
   - Append missing events from the current feature state up to `READINESS_EVENT`; never reorder or backfill past the current last event.
   - For projected events with required fields, use deterministic values derived from triage outputs:
     - `planreview_completed`: `--fq-count 0 --questions-asked "none"`
     - `plan_approved`: `--feasibility-required false`
     - `solutionreview_completed`: `--critical-count 0 --high-count 0`
     - `estimation_completed`: `--estimate-points "<added_task_points_total>"`
     - `tasking_completed`: `--task-count "<added_task_count>" --story-count "<affected_story_count_or_1>"`
   ...
   - Every append uses details indicating triage projection context:
     ...
     python scripts/pipeline_ledger.py append \
       ...
       --event "<event_name>" \
       --actor "<agent-id>" \
       --details "Projected by speckit.addtobacklog for NEXT_COMMAND=<next_command>"
     ```
