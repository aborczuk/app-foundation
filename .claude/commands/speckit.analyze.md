...
Run `.specify/scripts/python/check_prerequisites.py --json --require-tasks --include-tasks` once from repo root and parse JSON for FEATURE_DIR and AVAILABLE_DOCS. Derive absolute paths:
...
- Detect whether ingress/webhook/callback/public endpoint behavior is in scope using `spec.md`, `plan.md` when present, and `tasks.md`.
- If ingress applies:
...
#### A. Goal-to-Requirement Alignment

- Compare the one-line purpose, consumer/context, and user-story goals against the functional requirements set before doing downstream task-coverage analysis.
- Flag any user-visible goal, core user outcome, or promised end-to-end play loop that is not concretely represented in the functional requirements.
- Flag any functional-requirements set that is technically detailed but still fails to cover the declared feature goal end to end.
- Treat a material goal-to-FR miss as an upstream artifact problem: recommend returning to `specify` first, and to `plan` as well when plan/tasks were already derived from the incomplete FR set.
...
#### J. Local DB Transaction Integrity Gaps
...
#### K. External Ingress Readiness Gaps
