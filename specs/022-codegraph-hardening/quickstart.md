...
```bash
scripts/cgc_safe_index.py src/mcp_codebase
```
...
| `stale` | Local edits are newer than the last indexed snapshot. | Run `scripts/cgc_safe_index.py src/mcp_codebase` and retry. |
| `locked` | A stale owner or lock marker is present. | Stop the stale owner, then rerun `scripts/cgc_safe_index.py src/mcp_codebase`. |
...
| Locked Kuzu DB | Doctor/health check reports `locked` | Stop the stale process, then rerun `scripts/cgc_safe_index.py src/mcp_codebase`. |
...
- Treat `scripts/cgc_doctor.py` as the first stop for health checks; it is a read-only probe and does not rebuild the graph.
- Use `scripts/cgc_safe_index.py src/mcp_codebase` for recovery, not as a routine browse command.
...
- If the graph still fails smoke tests, rerun `scripts/cgc_safe_index.py src/mcp_codebase` before browsing
