...
feature_id: "031"
task_id: "T017"
task_summary: "Cap rerank requests to the shortlist-sized candidate window and route daemon scoring through a transport-neutral client in `scripts/read_code.py`"
primary_edit_seam: "scripts/read_code.py:_rerank_semantic_candidates"
related_paths: ["scripts/read_code.py"]
---

# HUD: T017 - Cap rerank requests to the shortlist-sized candidate window and route daemon scoring through a transport-neutral client in `scripts/read_code.py`

## Objective

Deliver `T017` so the declared seam satisfies: Cap rerank requests to the shortlist-sized candidate window and route daemon scoring through a transport-neutral client in `scripts/read_code.py`.
