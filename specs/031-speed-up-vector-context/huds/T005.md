...
Deliver `T005` so the declared seam satisfies: Restrict scoped code queries so irrelevant markdown retrieval is skipped in `scripts/read_code.py`.
...
- [FILL: write the actual task-local solution for `scripts/read_code.py:_query_semantic_anchor_candidate`.]
...
- [FILL: describe the exact task-seam behavior expected at `scripts/read_code.py:_query_semantic_anchor_candidate`.]
