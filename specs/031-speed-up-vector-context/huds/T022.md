...
feature_id: "031"
task_id: "T022"
task_summary: "Route semantic query requests through a transport-neutral daemon client contract in `scripts/read_code.py` and `src/mcp_codebase/index/reranker_runtime.py`"
primary_edit_seam: "scripts/read_code.py:_vector_query_candidates"
related_paths: ["scripts/read_code.py", "src/mcp_codebase/index/reranker_runtime.py"]
---

# HUD: T022 - Route semantic query requests through a transport-neutral daemon client contract in `scripts/read_code.py` and `src/mcp_codebase/index/reranker_runtime.py`

## Objective

Deliver `T022` so the declared seam satisfies: Route semantic query requests through a transport-neutral daemon client contract in `scripts/read_code.py` and `src/mcp_codebase/index/reranker_runtime.py`.
