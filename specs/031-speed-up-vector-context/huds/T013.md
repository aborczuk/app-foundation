...
Deliver `T013` so the declared seam satisfies: Update read-code help text or command documentation to describe the new broad-versus-scoped trust behavior in `scripts/read_code.py` and `specs/031-speed-up-vector-context/plan.md`.
...
- [FILL: write the actual task-local solution for `scripts/read_code.py:module:docstring`.]
...
- [FILL: describe the exact task-seam behavior expected at `scripts/read_code.py:module:docstring`.]
