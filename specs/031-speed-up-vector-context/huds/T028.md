...
feature_id: "031"
task_id: "T028"
task_summary: "Route `read_code` backend requests through the MCP-persistent server with bounded fallback in `scripts/read_code.py` and `tests/unit/test_read_code_reranker_daemon.py`"
primary_edit_seam: "scripts/read_code.py:_vector_query_candidates"
related_paths: ["scripts/read_code.py", "tests/unit/test_read_code_reranker_daemon.py"]
---

# HUD: T028 - Route `read_code` backend requests through the MCP-persistent server with bounded fallback in `scripts/read_code.py` and `tests/unit/test_read_code_reranker_daemon.py`

## Objective

Deliver `T028` so the declared seam satisfies: Route `read_code` backend requests through the MCP-persistent server with bounded fallback in `scripts/read_code.py` and `tests/unit/test_read_code_reranker_daemon.py`.
...
- [FILL: write the actual task-local solution for `scripts/read_code.py:_vector_query_candidates`.]
...
- [FILL: describe the exact task-seam behavior expected at `scripts/read_code.py:_vector_query_candidates`.]

## Primary Edit Seam

**File:Symbol**: `scripts/read_code.py:_vector_query_candidates`
