...
Deliver `T003` so the declared seam satisfies: Wire the new query-scope and trust helpers into the existing preflight contract in `scripts/read_code.py` and `scripts/read_code_health.py`.
