...
Deliver `T007` so the declared seam satisfies: Update `read_code context` output and inline behavior to preserve current seam selection while using the scoped fast path in `scripts/read_code.py`.
