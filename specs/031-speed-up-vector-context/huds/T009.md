...
Deliver `T009` so the declared seam satisfies: Rework broad-query routing so healthy-session trust can satisfy normal discovery reads in `scripts/read_code.py` and `scripts/read_code_health.py`.
