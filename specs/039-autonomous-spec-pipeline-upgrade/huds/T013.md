...
feature_id: "039"
task_id: "T013"
task_summary: "Add workflow phase adapter contracts in scripts/speckit_workflow_adapter.py for `orchestrate -> extract -> scaffold -> LLM Action -> validate -> emit/handoff` with declared inputs, template, output, mutation scope, runner, validator, diagnostics, and success event."
story_id: "US1"
primary_edit_seam: null
related_paths: ["orchestrate -> extract -> scaffold -> LLM Action -> validate -> emit/handoff"]
---

# HUD: T013 - Add workflow phase adapter contracts in scripts/speckit_workflow_adapter.py for `orchestrate -> extract -> scaffold -> LLM Action -> validate -> emit/handoff` with declared inputs, template, output, mutation scope, runner, validator, diagnostics, and success event.

## Objective

Deliver `T013` so the task scope satisfies: Add workflow phase adapter contracts in scripts/speckit_workflow_adapter.py for `orchestrate -> extract -> scaffold -> LLM Action -> validate -> emit/handoff` with declared inputs, template, output, mutation scope, runner, validator, diagnostics, and success event..
...
- [FILL: concrete change required in `orchestrate -> extract -> scaffold -> LLM Action -> validate -> emit/handoff`.]

## Touched Symbols

### Modify

- `orchestrate -> extract -> scaffold -> LLM Action -> validate -> emit/handoff` - [FILL: specific intended change.]
