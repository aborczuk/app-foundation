...
- **Constrained metric definition core** - Seed revenue, operating income, operating margin, improvement streak, and acceleration, then permit authorized analysts to define versioned metrics with approved filing-backed inputs and a typed declarative grammar. Preserve the metric-definition version on every observation; do not execute user code, SQL, or unbounded concept mapping. Covers FR-008 through FR-011 and FR-022 through FR-024. Requires custom server: yes. Source: supplied research on correctness boundaries plus feature refinement.
...
- **Metric core boundary**: Approved base-fact registry, built-in and user-defined versioned metric definitions, revenue and operating-income selectors, fiscal-period alignment, standalone-quarter derivation, margin, streak, acceleration, materiality thresholds, and immutable observation provenance.
