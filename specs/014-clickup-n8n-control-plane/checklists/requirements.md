# Specification Quality Checklist: ClickUp + n8n Operational Control Plane
...
- [x] If scope exclusions reference external tools/packages ("covered by X"), those tools have been verified as installable and functional — not just referenced by name
  - n8n: verified on npm registry (v2.14.2, `npm i n8n`)
  - ClickUp: verified public REST API with webhooks (developer.clickup.com)
...
- Two open questions (OQ-1: QA rework cycle cap, OQ-2: concurrent workflows) are surfaced but do not block planning — they have stated stakes and can be resolved during clarification.
